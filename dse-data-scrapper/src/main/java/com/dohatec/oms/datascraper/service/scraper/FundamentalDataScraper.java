...
public class FundamentalDataScraper {
    ...
    @Retryable(retryFor = { NetworkException.class,
            Exception.class }, maxAttempts = 3, backoff = @Backoff(delay = 2000, multiplier = 2.0))
    ...
    private CompanyDataDTO parseCompanyData(String html, String tradingCode) {
        try {
            ...
            // Capture labeled values that are rendered outside tables (e.g., AGM date
            // blocks)
            Elements labeledBlocks = document
                    .select("div.col-sm-6.pull-left, div.col-sm-6.pull-right, div.pull-left, div.pull-right");
    ...
    private Map<String, String> parseQuarterlyPerformanceTable(Element table) {
        ...
        try {
            ...
            // Parse quarter names and dates (starting from column 1, column 0 is
            // "Particulars")
     ...
     * This table has a complex structure with headers, rowspan, and multiple
     * sections
     * 
     * Table structure:
            ...
            for (Element row : rows) {
                ...
                if (cells.size() >= 2) {
                    ...
                    if (key.contains("Share Holding Percentage")) {
                        ...
                        String dateStr = extractDateFromText(key);
                        data.put("Shareholding_Pattern_Date", dateStr);
    ...
    /**
     * Extract date from text like "Share Holding Percentage [as on Dec 31, 2024
     * (year ended)]"
     * 
     * @param text the text containing date
     * @return extracted date string or year
     */
    private String extractDateFromText(String text) {
        try {
            // Try to extract content between "[as on " and "]"
            java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("\\[as on (.*?)\\]");
            java.util.regex.Matcher matcher = pattern.matcher(text);
            if (matcher.find()) {
                return matcher.group(1).trim();
            }

            // Fallback: Extract year if full date extraction fails
            ...
            log.debug("Could not extract date from text: {}", text);
        }
