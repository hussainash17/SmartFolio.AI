...
public class DsexIndexScheduler {
    ...
    /**
     * Scrape DSEX index shares trading codes and Market Summary
     ...
     */
    @Scheduled(cron = "${scraper.schedule.dsex-index-cron:0 0 15 * * SUN-THU}", zone = "${scraper.schedule.timezone:Asia/Dhaka}")
    public void scrapeDsexIndexShares() {
        try {
            log.info("=== Market Summary and Index scraping started ===");
            marketSummaryAndIndexScraper.scrapeMarketSummaryAndIndices();
            log.info("=== Market Summary and Index scraping completed ===");
        } catch (Exception e) {
            log.error("Market Summary and Index scraping failed", e);
