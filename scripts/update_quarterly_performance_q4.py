"""
Script to fetch Q4 (Annual) quarterly performance data from StockNow API and update the database.

This script:
1. Fetches all companies from the company table
2. For each company, calls the API to get Q4/Annual EPS data
3. Inserts/updates the quarterly_performance table with the data

Usage:
    # Update all companies (from project root directory):
    cd backend
    python ../scripts/update_quarterly_performance_q4.py
    
    # Update single company by trading code:
    cd backend
    python ../scripts/update_quarterly_performance_q4.py SQURPHARMA
    
    # Or if running from project root with backend in PYTHONPATH:
    PYTHONPATH=backend python scripts/update_quarterly_performance_q4.py
    PYTHONPATH=backend python scripts/update_quarterly_performance_q4.py SQURPHARMA
...
API Endpoint Pattern:
    https://stocknow.com.bd/api/v1/instruments/{COMPANY_CODE}/fundamentals/earning_per_share?groupBy=meta_date&yearLimit=9
...
async def fetch_quarterly_data_from_api(company_code: str) -> Optional[Dict[str, Any]]:
    ...
    """
    url = f"{API_BASE_URL}/{company_code}/fundamentals/{EPS_METRIC}?groupBy=meta_date&yearLimit=9"
...
async def update_single_company(trading_code: str) -> Dict[str, Any]:
    ...
    """
    print(f"🚀 Updating Q4 (Annual) quarterly performance for: {trading_code}")
    print("=" * 60)
...
async def main():
    """
    Main function to run the Q4 quarterly performance update script.
    ...
    if len(sys.argv) > 1:
        ...
        print(f"🎯 Single company mode (Q4/Annual): {trading_code}")
        print()
        ...
        print("\n✅ Q4 (Annual) Update complete!")
        return
    
    # Process all companies
    print("🚀 Starting Q4 (Annual) Quarterly Performance Data Update")
    print("=" * 60)
    ...
    with Session(engine) as db:
        ...
        print("\n✅ Q4 (Annual) Quarterly performance update complete!")
