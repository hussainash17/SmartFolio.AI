"""
Script to fetch Q3 quarterly performance data from StockNow API and update the database.

This script:
1. Fetches all companies from the company table
2. For each company, calls the API to get Q3 EPS data
3. Inserts/updates the quarterly_performance table with the data

Usage:
    # Update all companies (from project root directory):
    cd backend
    python ../scripts/update_quarterly_performance_q3.py
    
    # Update single company by trading code:
    cd backend
    python ../scripts/update_quarterly_performance_q3.py SQURPHARMA
    
    # Or if running from project root with backend in PYTHONPATH:
    PYTHONPATH=backend python scripts/update_quarterly_performance_q3.py
    PYTHONPATH=backend python scripts/update_quarterly_performance_q3.py SQURPHARMA
...
API Endpoint Pattern:
    https://stocknow.com.bd/api/v1/instruments/{COMPANY_CODE}/fundamentals/q3_eps_cont_op?groupBy=meta_date&yearLimit=9
...
async def update_single_company(trading_code: str) -> Dict[str, Any]:
    ...
    """
    print(f"🚀 Updating Q3 quarterly performance for: {trading_code}")
    print("=" * 60)
...
async def main():
    """
    Main function to run the Q3 quarterly performance update script.
    ...
    if len(sys.argv) > 1:
        ...
        print(f"🎯 Single company mode (Q3): {trading_code}")
        print()
    ...
    print("🚀 Starting Q3 Quarterly Performance Data Update")
    print("=" * 60)
    ...
    with Session(engine) as db:
        ...
        print("\n✅ Q3 Quarterly performance update complete!")
