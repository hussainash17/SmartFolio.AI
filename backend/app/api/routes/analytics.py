...
def get_portfolio_impact(
    ...
    if not portfolios:
        return {"events": [], "sector_composition": []}
    ...
    # Get unique stock IDs from user positions
    user_stock_ids = list(set([p.stock_id for p in positions]))
    
    # Fetch ONLY companies that the user holds (not all active companies)
    user_companies = session.exec(
        select(Company).where(Company.id.in_(user_stock_ids))
    ).all()
    company_map = {c.id: c for c in user_companies}
    
    # Batch fetch latest prices for user positions (optimized - single query with subquery)
    user_stock_prices = {}
    user_stock_data = {}  # Store full stock data for sector calculations
    
    if user_stock_ids:
        # Use a more efficient approach - fetch latest stock data for all user holdings
        for sid in user_stock_ids:
            latest = session.exec(
                select(StockData)
                .where(StockData.company_id == sid)
                .order_by(desc(StockData.timestamp))
                .limit(1)
            ).first()
            if latest:
                user_stock_prices[sid] = float(latest.last_trade_price)
                user_stock_data[sid] = latest

    # Calculate user sector allocation and positions
    user_positions = {}
    user_sector_value = {}
    ...
    for pos in positions:
        cid = pos.stock_id
        price = user_stock_prices.get(cid, float(pos.average_price))  # Fallback to avg price
        ...
        if comp and comp.sector:
            sector = comp.sector
            user_sector_value[sector] = user_sector_value.get(sector, 0.0) + val
            
            if sector not in user_positions:
                user_positions[sector] = []
            user_positions[sector].append({
                "symbol": comp.trading_code,
                "value": val,
    ...
    # 2. Calculate Sector Stats ONLY for sectors in user's portfolio
    # Get unique sectors from user holdings
    user_sectors = set(user_sector_value.keys())
    
    market_sector_stats = {}
    total_market_cap = 0.0
    
    # Get latest news for event correlation (filtered to user's sectors)
    today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
    recent_news = session.exec(
        select(News)
        .where(News.published_at >= today_start)
        .order_by(desc(News.published_at))
    ).all()
    
    # Calculate sector performance using ONLY stocks in those sectors
    # Fetch companies only from sectors the user is invested in
    sector_companies = session.exec(
        select(Company)
        .where(Company.is_active == True)
        .where(Company.sector.in_(user_sectors))
    ).all()
    
    # Batch fetch latest stock data for sector companies
    sector_company_ids = [c.id for c in sector_companies]
    sector_stock_data = {}
    
    if sector_company_ids:
        for cid in sector_company_ids:
            # Reuse data if already fetched for user holdings
            if cid in user_stock_data:
                sector_stock_data[cid] = user_stock_data[cid]
            else:
                sdata = session.exec(
                    select(StockData)
                    .where(StockData.company_id == cid)
                    .order_by(desc(StockData.timestamp))
                    .limit(1)
                ).first()
                if sdata:
                    sector_stock_data[cid] = sdata
    
    # Calculate sector performance stats
    for comp in sector_companies:
        if not comp.sector:
            continue
            
        sdata = sector_stock_data.get(comp.id)
        if not sdata or not sdata.last_trade_price:
            continue
            
        last = float(sdata.last_trade_price)
        open_p = float(sdata.open_price) if sdata.open_price else last 
        if open_p == 0:
            open_p = last
        ...
        sec = comp.sector
        if sec not in market_sector_stats:
            market_sector_stats[sec] = {
                "total_change_pct": 0.0,
                "count": 0,
                "mcap_proxy": 0.0 
            }
        
        market_sector_stats[sec]["total_change_pct"] += change_pct
        market_sector_stats[sec]["count"] += 1
        market_sector_stats[sec]["mcap_proxy"] += 1 
        total_market_cap += 1

    # 3. Detect Events & Calculate Impact
    events = []
    
    for sector, stats in market_sector_stats.items():
        if stats["count"] == 0: continue
        
        avg_change = stats["total_change_pct"] / stats["count"]
        
        # Determine Event Name & Type
        event_name = f"{sector} Movement"
        event_type = "technical"
        severity = "low"
        
        # Check News
        sector_news = [n for n in recent_news if n.title and (sector.lower() in n.title.lower() or (n.content and sector.lower() in n.content.lower()))]
        if sector_news:
             event_type = "news"
             event_name = f"{sector} News Impact"
        elif abs(avg_change) > 2.0:
            event_name = f"{sector} {'Rally' if avg_change > 0 else 'Selloff'}"
            severity = "high"
        elif abs(avg_change) > 1.0:
            event_name = f"{sector} {'Surge' if avg_change > 0 else 'Decline'}"
            severity = "medium"
        else:
            if abs(avg_change) < 0.5:
                continue
                
        # Calculate Impact on User
        user_val_in_sector = user_sector_value.get(sector, 0.0)
        impact_value = user_val_in_sector * (avg_change / 100)
        
        # Exposure Analysis
        user_weight = (user_val_in_sector / total_portfolio_value * 100) if total_portfolio_value > 0 else 0
        market_weight = (stats["mcap_proxy"] / total_market_cap * 100) if total_market_cap > 0 else 0
        
        # Generate Descriptions
        held_stocks = user_positions.get(sector, [])
        if not held_stocks:
            impact_desc = "No holdings in sector"
        elif len(held_stocks) <= 2:
            names = " + ".join([s["symbol"] for s in held_stocks])
            impact_desc = f"Your {names}"
        else:
            impact_desc = f"{len(held_stocks)} holdings affected"
            
        diff = user_weight - market_weight
        if abs(diff) < 5:
            exposure_desc = "Balanced exposure"
        elif diff > 0:
            exposure_desc = "Overweighting helping" if avg_change > 0 else "Overweight position hurting"
        else:
            exposure_desc = "Missed rally, consider adding?" if avg_change > 0 else "Underweight position protecting"
        ...
        events.append({
            ...
            "impact": {
                "value": round(impact_value, 2),
                "desc": impact_desc
            },
            "exposure": {
                "value": round(user_weight, 1),
                "market": round(market_weight, 1),
                "desc": exposure_desc
            }
        })

    events.sort(key=lambda x: abs(x["impact"]["value"]), reverse=True)
    
    # 4. Sector Composition for Visualization
    sector_visuals = []
    for sector, val in user_sector_value.items():
        weight = (val / total_portfolio_value * 100) if total_portfolio_value > 0 else 0
        m_stats = market_sector_stats.get(sector)
        perf = (m_stats["total_change_pct"] / m_stats["count"]) if m_stats and m_stats["count"] > 0 else 0.0
        
        sector_visuals.append({
            "name": sector,
            "weight": round(weight, 1),
            "performance": round(perf, 2)
        })
        
    sector_visuals.sort(key=lambda x: x["weight"], reverse=True)

    return {
        "events": events[:5], 
        "sector_composition": sector_visuals
    }


@router.get("/user/allocation")
def get_user_allocation(
    ...
    # Get all positions across all portfolios with stock information
    positions_query = session.exec(
        select(PortfolioPosition, Company)
        .join(Company, PortfolioPosition.stock_id == Company.id)
        .where(PortfolioPosition.portfolio_id.in_(portfolio_ids))
    ).all()
    ...
    # Aggregate positions by stock (sum quantities across portfolios)
    stock_aggregates = {}
    total_value = 0
    ...
    for position, stock in positions_query:
        ...
        if stock_id not in stock_aggregates:
            stock_aggregates[stock_id] = {
                "stock_id": stock_id,
                "symbol": stock.trading_code,
                "name": stock.company_name,
                "sector": stock.sector,
        ...
        stock_aggregates[stock_id]["quantity"] += quantity
        stock_aggregates[stock_id]["current_value"] += current_pos_value
        stock_aggregates[stock_id]["total_investment"] += total_investment
    ...
    stock_allocations = []
    for stock_data in stock_aggregates.values():
        unrealized_pnl = stock_data["current_value"] - stock_data["total_investment"]
        unrealized_pnl_percent = (unrealized_pnl / stock_data["total_investment"] * 100) if stock_data["total_investment"] > 0 else 0
        ...
        stock_allocations.append({
            ...
            "allocation_percent": round(stock_data["current_value"] / total_value * 100, 2),
    ...
    for allocation in stock_allocations:
        ...
        if sector not in sector_map:
            sector_map[sector] = {"value": 0, "stocks": []}
        sector_map[sector]["value"] += allocation["current_value"]
        if allocation["symbol"] not in sector_map[sector]["stocks"]:
            sector_map[sector]["stocks"].append(allocation["symbol"])
...
def get_user_holdings(
    ...
    # Collect company IDs
    company_ids = list(set([stock.id for _, stock in positions_query]))
    
    # Fetch latest stock data for these companies
    latest_prices = {}
    latest_changes = {}  # Store price change percentage
    ...
    if company_ids:
        for company_id in company_ids:
            ...
            if stock_data:
                if stock_data.last_trade_price:
                    latest_prices[company_id] = float(stock_data.last_trade_price)
                if stock_data.change_percent is not None:
                    latest_changes[company_id] = float(stock_data.change_percent)
    ...
    for position, stock in positions_query:
        ...
        existing_qty = stock_aggregates[stock_id]["quantity"]
        existing_investment = stock_aggregates[stock_id]["totalInvestment"]
        new_qty = quantity
        ...
        if total_qty > 0:
            ...
            if existing_qty > 0:
                existing_avg_price = existing_investment / existing_qty
                new_avg_price = new_investment / new_qty if new_qty > 0 else purchase_price
                weighted_price = (existing_avg_price * existing_qty + new_avg_price * new_qty) / total_qty
            else:
                weighted_price = purchase_price
            
            stock_aggregates[stock_id]["purchasePrice"] = weighted_price
            stock_aggregates[stock_id]["quantity"] = total_qty
            stock_aggregates[stock_id]["totalInvestment"] = existing_investment + new_investment
        else:
            stock_aggregates[stock_id]["purchasePrice"] = purchase_price
            stock_aggregates[stock_id]["quantity"] = quantity
            stock_aggregates[stock_id]["totalInvestment"] = total_investment
    ...
    for stock_data in stock_aggregates.values():
        ...
        purchase_price = stock_data["purchasePrice"]
        current_value = quantity * current_price
        total_investment = stock_data["totalInvestment"]
        
        # Calculate gain
        gain = current_value - total_investment
        gain_percent = (gain / total_investment * 100) if total_investment > 0 else 0
        
        # Get price change percentage from market data
        change_percent = latest_changes.get(UUID(stock_data["id"]), 0.0)
        ...
        holdings.append({
            ...
            "sector": stock_data["sector"] or "Unknown"
        })
