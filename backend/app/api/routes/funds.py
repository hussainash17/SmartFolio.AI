...
@router.get("/summary")
def get_funds_summary(current_user: CurrentUser, session: SessionDep):
    portfolios: List[Portfolio] = session.exec(
        select(Portfolio).where(Portfolio.user_id == current_user.id)).all() or []
    ...
    if open_buy_orders:
        ...
        rows: List[StockData] = session.exec(
            select(StockData).where(StockData.company_id.in_(company_ids)).order_by(StockData.company_id,
...
def list_transactions(current_user: CurrentUser, session: SessionDep, limit: int = 50, offset: int = 0):
    rows = session.exec(
...
@router.post("/deposit")
def deposit_funds(req: FundsChangeRequest, current_user: CurrentUser, session: SessionDep):
    amount = _safe_decimal(req.amount)
    if amount <= 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Amount must be positive")
    portfolio = _get_default_portfolio(session, current_user.id) if not req.portfolio_id else session.get(Portfolio,
                                                                                                          req.portfolio_id)
...
def withdraw_funds(req: FundsChangeRequest, current_user: CurrentUser, session: SessionDep):
    amount = _safe_decimal(req.amount)
    if amount <= 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Amount must be positive")
    portfolio = _get_default_portfolio(session, current_user.id) if not req.portfolio_id else session.get(Portfolio,
    ...
    if amount > available:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Insufficient available cash after reservations")
    portfolio.cash_balance = _safe_decimal(portfolio.cash_balance) - amount
    tx = AccountTransaction(
        user_id=current_user.id,
        portfolio_id=portfolio.id,
...
def update_funds_settings(req: FundsSettingsRequest, current_user: CurrentUser, session: SessionDep):
