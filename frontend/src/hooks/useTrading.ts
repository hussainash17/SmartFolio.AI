...
export function useTrading() {
  ...
  // Prefer API-provided market list with realtime updates; fall back to mock data
  const { data: apiMarketData } = useQuery({
  ...
  useEffect(() => {
    ...
    const disconnect = connectRealtime(wsUrl, () => {
      // A message indicates data changed; the component using this hook can re-render based on query cache changes
      // We do not have direct access to queryClient here without passing context, so a consumer can handle refetch.
    });
    return () => disconnect();
  }, []);

  const marketData = useMemo(() => apiMarketData ?? MOCK_MARKET_DATA, [apiMarketData]);
